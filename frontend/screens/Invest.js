...
import { InvestmentCard, Divider, Button, Block, Text, Input } from '../components';
...
class Invest extends Component {
  ...
  changeInvestmentPercentage = (key, value) => {
    ...
    const total = Object.values(investments).reduce((prev, amount) => { prev += amount; return prev }, 0)
  ...
  getInvestment = percentage => this.toCurrency(percentage * this.state.investmentAmount / 100)
  getRetirementValue = value => this.toCurrency((value * this.state.investmentAmount / 100) * (interest_rate ^ years_till_retirement))
  ...
  render() {
    ...
    return (
      <Block>
        <ScrollView showsVerticalScrollIndicator={false}>
          <Block style={styles.inputs}>
            ...
            <Block row space="between" margin={[10, 0, 0, 0]}>
              ...
              />
              <Text small right>total: {total}%</Text>
            </Block>
          </Block>
          <Divider />
          ...
          <Block padding={15} margin={[0, 0, 45, 0]}>
            {funds.map(fund => {
              ...
              return <InvestmentCard
                ...
                onSliderChange={val => this.changeInvestmentPercentage(fund.key, val)}
                atRetirement={fund.atRetirement && this.getRetirementValue(investment)}
              />
            })}
          </Block>
        ...
        <Block padding={[10, theme.sizes.base * 2]} bottom flex={0.6} margin={[0, 0, 10, 0]}>
